...
var loadInterval = setInterval(function() {
  ...
  function getTime() {
    ...
    if (timeNow > 1505671200) {
      if (state !== 'presaleended') {
        state = 'presaleended';
        document.getElementById('presale-info').textContent = 'Our ICO will start on xxxxx';
        document.getElementById('icotitle').textContent = 'YUPIE pre-sale over'
      }
      timestamp = 1509523200 - timeNow;
    } else if (timeNow > 1502784000) {
      if (state !== 'presale') {
        state = 'presale';
        document.getElementById('presale-info').textContent = ''; // 'Wallet address: 0x0F33bb20a282A7649C7B3AFf644F084a9348e933';
        document.getElementById('presale-info2').innerHTML = '<p class="ico-label-start">ico starts on 1.11.2017</p><p>price 1.000 YUPIES = 1 ETH</p>';
        document.getElementById('presale-info3').innerHTML = ''; // '<p class="ico-label-terms"><small><small>By purchasing YUPIES you agree to our <a class="ico-label-termslink" href="https://ico.crowdholding.com/docs/crowdholding%5Fllc%5Fterms%5Fagreement.pdf"><u>Crowdholding LLC Terms & Agreement</u></a></small></small></p>';
        document.getElementById('icotitle').textContent = 'YUPIE PRE-ICO IS LIVE!'
      }
      timestamp = 1505671200 - timeNow;
    } else {
      timestamp = 1502784000 - timeNow;
    }
  ...
  var sections = {};
  var sectionNames = ['home', 'project', 'faq', 'ico', 'team', 'roadmap', 'advisors', 'investors', 'documents', 'media', 'subscribe'];

  function sectionAnimations() {
    requestAnimationFrame(function() {
      for (var section in sectionNames) {
        var name = sectionNames[section];
        var s = sections[name];
        if (!s) {
        ...
        if (s && s.node) {
          var nodep = s.node.getBoundingClientRect();
          if (s.visible === false) {
            if (nodep.top < window.innerHeight) {
              s.visible = true;
              s.node.style.opacity = '0';
              s.node.style.transform = 'translateY(100px)';
              setTimeout(function() {
                this.n.style.transition = 'all 0.8s';
                setTimeout(function() {
                  this.n.style.transform = 'translateY(0px)';
                  this.n.style.opacity = '1';
                }.bind({
                  n: this.n
                }), 100);
              }.bind({
            ...
            if (nodep.top < window.innerHeight - 20) {
              s.visible = true;
              s.node.style.opacity = '1';
              s.node.style.transform = 'translateY(0px)';
              return;
            }
            s.visible = false;
            s.node.style.opacity = '0';
  ...
  // Content Fade
  setTimeout(function() {
    // document.getElementById('companytitle').style.transform = 'translateY(0px)';
    document.getElementById('chlogo').style.transform = 'rotate(180deg)';
  }, 1500);
  setTimeout(function() {
    // document.getElementById('svgwrapper').style.transform = 'translateX(-40%)';
    document.getElementById('main-fade').style.opacity = '1';
    document.getElementById('social-bar').style.opacity = '1'
  }, 2000)
...
setTimeout(function() {
  ...
  window.$zopim || (function(d, s) {
    ...
    $.src = 'https://v2.zopim.com/?dcddMads9dKCbFKQ31NoBcyxEiMZL4ZN';
    z.t = +new Date;
    $.
...
setTimeout(function() {
  window.particleground(document.getElementById('pg-canvas'), {
    dotColor: '#F0F0F0',
    lineColor: '#F0F0F0'
  });
})
